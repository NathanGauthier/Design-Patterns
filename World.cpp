...
void World::load(SDL_Renderer* renderer)
{
	...
	sand = TileData("sand.bmp", 2, renderer);
	rock = TileData("rock.bmp", 10, renderer);
	grid = generate();
}
...
array<array<Tile*, 25>, 20> World::generate()
{
	...
	array<array<int, 25>, 20> gridData{ {
	...
	for (auto row : rGrid)
	{
		for (auto tile : row)
		{
			TileData* data = chooseTileData(gridData[x][y]);
			tile = new Tile(x, y, data);
			++x;
		}
		x = 0;
		++y;
	...
	for (int row = 0; row < 20; ++row)
	{
		...
		{
			TileData* type = chooseTileData(gridData[row][col]);
